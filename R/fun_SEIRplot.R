...
SEIRplot <- function(pars_estimate, file_name, init_settings, panel_B_R_ylim=4,
 ...
 ##
  ...
  #estSEAIP_mat <- SEIRsimu(pars = pars_estimate, init_settings = init_settings, num_periods = n.stage)
  ...
  library(ggplot2)
    ...
    p <- ggplot(data,aes(x=date))+geom_line(aes(y = Prevalance))+
      ...
      theme_Publication()+
      ggtitle(paste0("Prevalance in ",paste0(statename[i1])," Ascertainment ", overall_ascertainment," ls",least_square))
