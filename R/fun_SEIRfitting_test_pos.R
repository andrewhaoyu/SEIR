...
Func_list = function(method){
  default_pars_density <- function(pars) {
    ...
    #beta2 for test positive rate
    d_vec[n.stage+3] = dnorm(pars[n.stage+3],delta_mean, delta_sd, log = T)
    phi = pars[n.stage+4]
    #d_vec[2*n.stage+1] = dgamma(phi,gamma_shape,gamma_rate,log =T)
    d_vec[n.stage+4] = dinvgamma(phi,shape = gamma_shape,
  ...
  default_pars_sampler <- function(n.stage=n.stage) {
    ...
    s_vec[, n.stage+3] <- rnorm(1, delta_mean, delta_sd)
    s_vec[, n.stage+4] <- rinvgamma(1,
                                    shape = gamma_shape,
                                    rate = gamma_rate)
  ...
  pars_name=c(paste0("b",c(1:n.stage)),"c0","c1","c2",paste0("phi"))
